...
namespace Ruffles.Channeling
{
	internal class ChannelPool
	{
		...
		private readonly ConcurrentCircularQueue<ReliableSequencedChannel> _reliableSequencedChannels;
		...
		private readonly ConcurrentCircularQueue<UnreliableSequencedChannel> _unreliableSequencedChannels;
		...
		internal IChannel GetChannel(ChannelType type, byte channelId, Connection connection, SocketConfig config, MemoryManager memoryManager)
		{
			...
			{
				if (_reliableSequencedChannels != null && _reliableSequencedChannels.TryDequeue(out ReliableSequencedChannel channel))
				{
					channel.Assign(channelId, connection, config, memoryManager);
			...
			if (type == ChannelType.UnreliableSequenced)
			{
				if (_unreliableSequencedChannels != null && _unreliableSequencedChannels.TryDequeue(out UnreliableSequencedChannel channel))
				{
					channel.Assign(channelId, connection, config, memoryManager);
				...
				return new UnreliableSequencedChannel(channelId, connection, config, memoryManager);
			}
