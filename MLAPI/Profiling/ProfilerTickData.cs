...
namespace MLAPI.Profiling
{
	...
	public class ProfilerTick
	{
		...
		internal void StartEvent(TickType type, uint bytes, string channelName, string messageType, string time)
		{
			TickEvent tickEvent = new TickEvent()
			{
				Bytes = bytes,
				Time = string.IsNullOrEmpty(time) ? "NONE" : time,
		...
		public static TickEvent FromStream(Stream stream)
		{
			using (PooledBitReader reader = PooledBitReader.Get(stream))
			{
				TickEvent @event = new TickEvent
				{
					...
					Time = reader.ReadStringPacked().ToString(),
